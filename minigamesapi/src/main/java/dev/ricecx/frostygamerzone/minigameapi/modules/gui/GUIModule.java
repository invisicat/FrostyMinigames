package dev.ricecx.frostygamerzone.minigameapi.modules.gui;
...
import dev.ricecx.frostygamerzone.minigameapi.modules.gui.core.menu.FrostMenuListener;
import dev.ricecx.frostygamerzone.minigameapi.modules.gui.core.menu.FrostOpenMenu;
...
public class GUIModule extends Module {
    ...
    /**
     * The defaultPaginationButtonBuilder is the plugin-wide {@link FrostPaginationButtonBuilder}
     ...
     * on either the inventory class ({@link FrostMenu}) or your plugin's instance of
     * {@link GUIModule}.
     */
    private FrostPaginationButtonBuilder defaultPaginationButtonBuilder = (type, inventory) -> {
        switch (type) {
            case PREV_BUTTON:
                if (inventory.getCurrentPage() > 0) return new FrostButton(new ItemBuilder(Material.ARROW)
            ...
            case CURRENT_BUTTON:
                return new FrostButton(new ItemBuilder(Material.NAME_TAG)
            ...
            case NEXT_BUTTON:
                if (inventory.getCurrentPage() < inventory.getMaxPage() - 1) return new FrostButton(new ItemBuilder(Material.ARROW)
     ...
     * this class by calling {@link #create(String, int)} on the static {@link GUIModule} field.
     *
     ...
     * <br><br>
     * Thus, the design whereby this class is registered as a static field on a {@link CorePlugin}
     * instance and serves as a proxy for creating ({@link FrostMenu}) inventories and an instance
     * of the {@link FrostMenu} registered with that plugin seemed like a good way to try
     * and minimize the inconvenience of the approach.
     *
     */
    public GUIModule() {
        CorePlugin.getInstance().registerListeners(new FrostMenuListener());
     ...
     */
    public FrostMenu create(String name, int rows) {
     ...
     * @return The created inventory.
     */
    public FrostMenu create(String name, int rows, String tag) {
        return new FrostMenu(name, rows, tag);
    }

    /**
     * @see GUIModule#blockDefaultInteractions
     *
    ...
    /**
     * Returns the value of {@link GUIModule#blockDefaultInteractions} for this plugin.
     *
    ...
    /**
     * @see GUIModule#enableAutomaticPagination
     *
    ...
    /**
     * Returns the value of {@link GUIModule#enableAutomaticPagination} for this plugin.
     *
    ...
    /**
     * @see GUIModule#defaultPaginationButtonBuilder
     *
     * @param defaultPaginationButtonBuilder The default pagination button builder used for GUIs.
     */
    public void setDefaultPaginationButtonBuilder(FrostPaginationButtonBuilder defaultPaginationButtonBuilder) {
     ...
     */
    public FrostPaginationButtonBuilder getDefaultPaginationButtonBuilder() {
     ...
     *
     * This returns a list of {@link FrostOpenMenu} which simply stores the
     ...
     * @return A list of {@link FrostOpenMenu} whose inventories have the specified tag.
     */
    public List<FrostOpenMenu> findOpenWithTag(String tag) {

        List<FrostOpenMenu> foundInventories = new ArrayList<>();
        ...
        for (Player player : Bukkit.getServer().getOnlinePlayers()) {
            ...
            if (topInventory.getHolder() != null && topInventory.getHolder() instanceof FrostMenu) {
                // and the SGMenu has the tag matching the one we're checking for,
                FrostMenu inventory = (FrostMenu) topInventory.getHolder();
                if (inventory.getTag().equals(tag))
                    // add the SGMenu to our list of found inventories.
                    foundInventories.add(new FrostOpenMenu(inventory, player));
            }
        }
